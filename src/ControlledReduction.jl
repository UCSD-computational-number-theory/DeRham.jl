        ...
        while j <= length(costadata_arr)
            if all(costadata_arr[i][1][1] .== costadata_arr[j][1][1])
                costadata_arr[i] = (
                    (
                        costadata_arr[i][1][1],
                        costadata_arr[i][1][2] + costadata_arr[j][1][2],
                    ),
                    costadata_arr[i][2][1],
                )
                deleteat!(costadata_arr, j)
...
function default_context(matspace, Ruv, params)
    ...
    if params.use_gpu == true && (ZZ(2)^25 < m < ZZ(2)^106)
        ...
        A = GPUFiniteFieldMatrices.KaratsubaZeros(
            Float64,
            g_length,
            g_length,
            N1,
        ...
        B = GPUFiniteFieldMatrices.KaratsubaZeros(
            Float64,
            g_length,
            g_length,
            N1,
        ...
        temp = GPUFiniteFieldMatrices.KaratsubaZeros(
            Float64,
            g_length,
            g_length,
            N1,
        ...
        g_temp =
            GPUFiniteFieldMatrices.KaratsubaZeros(Float64, g_length, N1, N2, N1*N2, true)
        GPUFiniteFieldMatrices.initialize_plan!(g)
        GPUFiniteFieldMatrices.initialize_plan!(g_temp)

        return ControlledReductionContext{KaratsubaMatrix{Float64},KaratsubaVector{Float64}}(
            Ruv,
            A,
            B,
            temp,
    ...
    elseif params.use_gpu == true
        ...
        return ControlledReductionContext{CuModMatrix{Float64},CuModVector{Float64}}(
            Ruv,
            A,
            B,
            temp,
        ...
        if params.always_use_bigints || ZZ(2)^64 < ZZ(m)
            ...
            g = [ZZ(0) for i = 1:g_length]
            g_temp = [ZZ(0) for i = 1:g_length]
        else
            # Small modulus
...
>>>if you don't know what this is, ignore it.
"""
function pregen_default_context(
    n,
    d,
    p,
    S;
...
function select_Ruv_PEP(n, d, S, params, compute, lazy, oscar_matspace, cache)
    ...
    elseif 3 < n && d == 3 && S == [n] && params.use_gpu && (ZZ(2)^25 < m < ZZ(2)^106) #4 < n
        ...
        cpu_Ruv =
            LazyPEP{Matrix{Float64}}(compute_float, eagerVs = eager_Vs, usethreads = false)
        ...
        if (3 < params.verbose)
            create_gpu =
                matrices -> begin
                    ...
                    CUDA.@time KaratsubaMatrix.(
                        Float64,
                        CuModMatrix.(matrices, N1*N2, elem_type = Float64),
                        N1,
                        N2,
                        N1*N2,
                    )
        ...
        Ruv = CachePEP{Matrix{Float64},KaratsubaMatrix{Float64}}(
            cpu_Ruv,
            create_gpu,
            convert_gpu,
            maxsize,
    ...
    elseif 3 < n && d == 3 && S == [n] && params.use_gpu #4 < n
        ...
        Ruv = CachePEP{Matrix{Float64},CuModMatrix{Float64}}(
            cpu_Ruv,
            create_gpu,
            convert_gpu,
            maxsize,
    ...
    elseif params.use_gpu && (ZZ(2)^25 < m < ZZ(2)^106)
        compute_gpu_karatsuba = V -> @. KaratsubaMat(float_entries(compute(V)))
        Ruv = EagerPEP{KaratsubaMatrix{Float64}}(
            cache[d],
            compute_gpu_karatsuba,
            usethreads = false,
        )

    elseif params.use_gpu
        Ruv = EagerPEP{CuModMatrix{Float64}}(
            cache[d],
            compute_gpu,
            usethreads = params.use_threads,
        )
    ...
    elseif lazy
        ...
        Ruv = EagerPEP{typeof(oscar_matspace())}(
            cache[d],
            compute,
            usethreads = params.use_threads,
        )
...
function pregen_select_Ruv_PEP(n, d, S, params, oscar_matspace)
    ...
    if 3 < n && d == 3 && S == [n] && params.use_gpu && (ZZ(2)^25 < m < ZZ(2)^106) #4 < n
        ...
        create_gpu =
            matrices -> KaratsubaMatrix.(
                Float64,
                CuModMatrix.(matrices, N1, elem_type = Float64),
                N1,
                N2,
                N1*N2,
            )
