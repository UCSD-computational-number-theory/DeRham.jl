...
function eval_to_linear_gpu!(A, B, temp, matrices, U, V)
    ...
    @cuda threads=threads blocks=blocks eval_to_linear_kernel!(
        A.data,
        B.data,
        mat_tuple,
        tuple(U...),
...
function eval_to_linear_karatsuba_kernel!(
    Adata1,
    Adata2,
    Bdata1,
    Bdata2,
    ...
    for k = 2:length(matrices1)

        m1 = matrices1[k][i]
        m2 = matrices2[k][i]

        temp1, temp2 =
            GPUFiniteFieldMatrices.karatsuba_scalar_multiply_helper(m1, m2, U[k-1], N1, N2)
        bb1, bb2 =
            GPUFiniteFieldMatrices.karatsuba_add_helper(bb1, bb2, temp1, temp2, N1, N2)

        temp1, temp2 =
            GPUFiniteFieldMatrices.karatsuba_scalar_multiply_helper(m1, m2, V[k-1], N1, N2)
        aa1, aa2 =
            GPUFiniteFieldMatrices.karatsuba_add_helper(aa1, aa2, temp1, temp2, N1, N2)
    end
...
function eval_to_linear_gpu_karatsuba!(A, B, temp, matrices, U, V)
    ...
    @cuda threads=threads blocks=blocks eval_to_linear_karatsuba_kernel!(
        A.data1.data,
        A.data2.data,
        B.data1.data,
        B.data2.data,
