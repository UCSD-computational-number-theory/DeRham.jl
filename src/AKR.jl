...
function reducechain_akr(g, m, f, p, picache, params)
    ...
    while m > n
        ...
        g = derivative(
            vector_to_polynomial(gtemp[1:len], n, d*(m-1)-n, PR, params.termorder),
            1,
        )
        for i = 1:n
            gpolytemp = derivative(
                vector_to_polynomial(
                    gtemp[(i*len+1):((i+1)*len)],
                    n,
                    d*(m-1)-n,
                    PR,
                    params.termorder,
...
function reducetransform_akr(FT, N_m, S, f, pseudoInverseMat, p, cache, params)
    ...
    for i = (n+1):highm
        ...
        for i = 1:nrows(pi_new)
            for j = 1:ncols(pi_new)
