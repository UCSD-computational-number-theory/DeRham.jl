...
function precision_information_auto(f, field; basis = nothing, verbose = 0)
    if characteristic(field) != characteristic(parent(f))
        error(
            "Field characteristic $(characteristic(field)) does not match polynomial base field characteristic $(characteristic(parent(f))).",
        )
    end
...
function precision_information_max_auto_r_m(f, r_m; basis = nothing, verbose = 0)
    (hodge_polygon, r_m_auto, _, _) =
        precision_information_auto(f; basis = basis, verbose = verbose)
    (_, r_m_user, _, _) =
        precision_information_user_r_m(f, r_m; basis = basis, verbose = verbose)
