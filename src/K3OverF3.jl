...
function zeta_coefficients_k3f3(
    f;
    S = [-1],
    verbose = false,
    precise = true,
    ...
    if (0 < verbose)
        ...
        @time f_changed, f, pseudo_inverse_mat_new =
            find_Ssmooth_model(f, M, S, params, changef)
    else
    ...
    Reductions =
        reducetransform(FBasis, N_m, S, fLift, pseudo_inverse_mat, p, params, cache)
